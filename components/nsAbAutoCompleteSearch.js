...
nsAbAutoCompleteSearch.prototype = {
  ...
  classDescription: "Address Book Autocomplete2",
    ...
    while (childCards.hasMoreElements()) {
      ...
      if (card instanceof Components.interfaces.nsIAbCard) {
        if (card.isMailList)
		  ...
		  var enableGlobal =  prefs.getBoolPref("morecols.autocomplete.use_multiple_emails");
    ...
    while (childCards.hasMoreElements()) {
      ...
      if (card instanceof Components.interfaces.nsIAbCard) {
        if (card.isMailList)
		  ...
		  for (var i=1;i<6;i++) {
			email = card.getProperty("MFFABemail"+i, "");
			if (email && email.toLocaleLowerCase().lastIndexOf(fullString, 0) == 0)
				this._addToResult(commentColumn, directory, card, email, false, result);
		  }
        }
      }
  ...
  _addToResult: function _addToResult(commentColumn, directory, card,
	...
	for (i=0;i<t.length;i++) {
	   if (i > 0)
			sep = ",";
		if (this._parser.makeFullAddress)
			emailAddress += sep + this._parser.makeFullAddress(card.displayName,
                                   card.isMailList ?
  ...
  startSearch: function startSearch(aSearchString, aSearchParam,
    ...
    else
    {
      ...
      if (is27())
		 var searchQuery = "(or(DisplayName,c,@V)(FirstName,c,@V)(LastName,c,@V)(NickName,c,@V)(and(IsMailList,=,TRUE)(Notes,c,@V)))";
      else
		var searchQuery = "(or(DisplayName,bw,@V)(FirstName,bw,@V)(LastName,bw,@V)(NickName,bw,@V)(and(IsMailList,=,TRUE)(Notes,bw,@V)))";
     
      searchQuery = searchQuery.replace(/@V/g, encodeURIComponent(fullString));

      if (firstWord && rest) {
	 if (is27())
		 var searchFNLNPart = "(or(and(FirstName,c,@V1)(LastName,c,@V2))(and(FirstName,c,@V2)(LastName,c,@V1)))";
	else
	      var searchFNLNPart = "(or(and(FirstName,bw,@V1)(LastName,bw,@V2))(and(FirstName,bw,@V2)(LastName,bw,@V1)))";
	...
	if (is27()) {
		if (prefSvc.getBoolPref("morecols.autocomplete.use_additional_emails"))
			var emailSearchQuery = "?(or(PrimaryEmail,c,@V)(SecondEmail,c,@V)(MFFABemail1,c,@V)(MFFABemail2,c,@V)(MFFABemail3,c,@V)(MFFABemail4,c,@V)(MFFABemail5,c,@V))";
		else
			var emailSearchQuery = "?(or(PrimaryEmail,c,@V)(SecondEmail,c,@V))";
	}
	else {
		if (prefSvc.getBoolPref("morecols.autocomplete.use_additional_emails"))
			var emailSearchQuery = "?(or(PrimaryEmail,bw,@V)(SecondEmail,bw,@V)(MFFABemail1,bw,@V)(MFFABemail2,bw,@V)(MFFABemail3,bw,@V)(MFFABemail4,bw,@V)(MFFABemail5,bw,@V))";
		else
			var emailSearchQuery = "?(or(PrimaryEmail,bw,@V)(SecondEmail,bw,@V))";
	}
